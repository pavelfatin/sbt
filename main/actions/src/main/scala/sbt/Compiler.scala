...
object Compiler
{
	...
	def apply(in: Inputs, log: Logger): Analysis =
	{
		...
		agg(Some(scalac), javac, sources, classpath, CompileOutput(classesDirectory), cache, None, options, javacOptions, analysisMap, definesClass, new LoggerReporter(maxErrors, log, sourcePositionMapper), None, order, skip)(log)
	}
